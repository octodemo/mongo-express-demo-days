...
const router = function (config) {
  ...
  function encrypt(text){
      var cipher = crypto.createCipher('aes-256-ctr', process.env.crypto_key);
      return cipher.update(text, 'utf8', 'hex') + cipher.final('hex');
    }
  
  appRouter.post('/db/search', mongoMiddleware, (req, res) => {
    const items = [
                  "item 1 foo bar",
                  "item 2 dev hello",
                  "item 3 for sync"
    ...
    const search = req.param("search").replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); // $& means the whole matched string;
    const username = req.param("username");
    const pw = req.param("password");
    
    // authentication logic
    
    const re = new RegExp(search); 
    const filteredItems = items.filter(value => re.test(value));
    
    res.cookie('username', encrypt(username));
    res.cookie('password', encrypt(pw));
    
    res.send(200, filteredItems);
  });
